    ...
    <div :class="bem.ify(block, 'form-actions')">
      <div :class="bem.ify(block, 'form-actions')">
        ...
        <template v-else>
          ...
          <button class="button" @click.prevent="$emit('edit-cancel')">
...
export default {
  ...
  data() {
    return {
      ...
      enumerationInput: this.$props.task.project.activitiesPerRole[0].id,
  ...
  computed: {
    ...
    canSave() {
      const hoursParsed = parseFloat(this.hours.replace(",", "."));
